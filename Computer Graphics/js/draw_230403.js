...
boxData.push({
    minPt: new THREE.Vector2(150, 150),
    maxPt: new THREE.Vector2(350, 350),
    cr: "rgb(" +
...
circleData.push({
    ctr: new THREE.Vector2(50, 50), radius: 10, cr: "rgb(" +
        Math.floor(Math.random() * 256) +
        "," +
        Math.floor(Math.random() * 256) +
        "," +
...
circleData.push({
    ctr: new THREE.Vector2(400, 100), radius: 50, cr: "rgb(" +
        Math.floor(Math.random() * 256) +
        "," +
        Math.floor(Math.random() * 256) +
        "," +
...
circleData.push({
    ctr: new THREE.Vector2(450, 450), radius: 30, cr: "rgb(" +
        Math.floor(Math.random() * 256) +
        "," +
        Math.floor(Math.random() * 256) +
        "," +
...
function draw_box(boxData) {
    ...
    if (
        mouseX >= boxData.minPt.x &&
        mouseX <= boxData.maxPt.x &&
        mouseY >= boxData.minPt.y &&
        mouseY <= boxData.maxPt.y
    ) {
        ctx.fillStyle = boxData.cr;
        isFill = true;
    }
...
function draw_circle(circleData) {
    ...
    if (
        Math.pow(mouseX - circleData.ctr.x, 2) +
        Math.pow(mouseY - circleData.ctr.y, 2) <
        Math.pow(circleData.radius, 2)
    ) {
        ctx.fillStyle = circleData.cr;
        isFill = true;
    }
    ctx.beginPath();
    ctx.arc(
